...
2. 正常に起動すると slider のプログラムによって LCD の表示がはじまる
  ...
  1. 基本的に時計として時刻を表示する。ただし、RPi は内部時計を持たないため、この次点で時刻は正しくない
...
どこにもつながっていない閉じたネットワークなので安心して利用できるメリットがある  
...
3. 原始的な時刻補正だが、ネットワークが近いので小さい遅延で時刻の同期が出来る。RTC も NTP も使えない環境で便利
